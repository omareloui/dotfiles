...
fn init(config_dir: RString) -> Config {
    match fs::read_to_string(format!("{}/websearchplus.ron", config_dir)) {
...
fn get_matches(input: RString, config: &Config) -> RVec<Match> {
    if !input.starts_with(&config.prefix) {
        ...
        config
            ...
            .enumerate()
            .map(|(_, engine)| Match {
                title: input.trim_start_matches(&config.prefix).trim_start_matches(&engine.secondary_prefix()).trim_start().into(),
