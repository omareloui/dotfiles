...
local mediainfo_image_rec_mutliplier = 1.333333333
...
function M:peek(job)
	...
	-- NOTE: Workaround case video.lua doesn't generate preview image because of `skip` overflow video duration
	...
	ya.preview_widget(job, {
		ui.Text(lines)
			:area(ui.Rect({
				x = job.area.x,
				y = job.area.y + image_height * mediainfo_image_rec_mutliplier,
				w = job.area.w,
				h = job.area.h - image_height * mediainfo_image_rec_mutliplier,
			}))
