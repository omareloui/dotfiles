{
  writeShellApplication,
  dunst,
  acpi,
  ripgrep,
}:
writeShellApplication {
  name = "batsuspend";
  runtimeInputs = [dunst acpi ripgrep];
  text =
    /*
    bash
    */
    ''
      ${import ../utils/ansi.nix}

      ${import ./config.nix}

      version=1.0.0

      LONGOPTS=help
      OPTIONS=h

      PARSED=$(getopt --options=$OPTIONS --longoptions=$LONGOPTS --name "$0" -- "$@")
      eval set -- "$PARSED"

      while true; do
        case "$1" in
        -h | --help)
          echo -e ""
          echo -e "  ''${BOLD}Usage:''${END_BOLD}    $(basename "$0") ''${YELLOW}[''${B_RED}options''${DARK_GRAY}...''${YELLOW}]''${RESET}"
          echo -e "  ''${BOLD}Version:''${RESET}  ''${YELLOW}$version''${RESET}"
          echo -e ""
          echo -e "  ''${BOLD}Description:''${RESET}"
          echo -e ""
          echo -e "    ''${B_GRAY}Suspense the laptop depending on the battery percentage (requires a cron job).''${RESET}"
          echo -e ""
          echo -e "  ''${BOLD}Options:''${RESET}"
          echo -e ""
          echo -e "    ''${BLUE}-h''${RESET}, ''${BLUE}--help''${RESET}       ''${B_RED}-''${RESET} Show this help and exit."
          echo -e ""
          echo -e "  ''${BOLD}Examples:''${RESET}"
          echo -e ""
          echo -e "    ''${RED}\$''${RESET} ''${B_MAGENTA}$(basename "$0")''${RESET}"
          echo -e ""
          exit 0
          ;;
        --)
          shift
          break
          ;;
        *)
          echo -e "''${RED}Error:''${RESET} Programming error"
          exit 3
          ;;
        esac
      done

      is_charging="$(acpi | rg 'Battery 1' | rg -c "Charging")"
      current_percentage="$(acpi | rg 'Battery 1' | rg -o --pcre2 '[0-9]{1,3}(?=%)')"

      if ((is_charging==0 && current_percentage<=SUSPEND_ON)); then
        dunstify \
          "Going to sleep..." \
          "$current_percentage% of battery remaining." \
          -i "$HOME/.local/share/icons/sleep.png" \
          -u critical
        sleep 3
        systemd-run --user systemctl hybrid-sleep
      fi
    '';
}
